...
if(strlen($_REQUEST["cName"])==0){
	...
	$sql="insert into customer (Name,username,Gender,Phone,Address,email,DOB) values('".$_REQUEST["cName"]."','".$_REQUEST["userCname"]."','".$_REQUEST["gender"]."','".$_REQUEST["phn"]."','".$_REQUEST["address"]."','".$_REQUEST["email"]."','".$_REQUEST["bday"]."')";
	echo $sql;
	$conn = mysqli_connect("localhost", "root", "","webtech");
	$result = mysqli_query($conn, $sql)or die(mysqli_error($conn));
	$c=mysqli_affected_rows($conn);
	
	$sql="insert into login values('".$_REQUEST["userCname"]."','".md5($_REQUEST["pss"])."',0)";
	echo $sql;
	$conn = mysqli_connect("localhost", "root", "","webtech");
	$result = mysqli_query($conn, $sql)or die(mysqli_error($conn));
	$c=mysqli_affected_rows($conn);
